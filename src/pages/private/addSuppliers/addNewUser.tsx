...
export const AddUserPerOffice: React.FC<{
  editId: any;
  setIsUserPopup: any;
  setDropdownOpen: (open: boolean) => void;
  setIsRefetch: any;
}> = ({ editId, setIsUserPopup, setDropdownOpen, setIsRefetch }) => {
  ...
  const userId = pathSegments.length > 3 ? pathSegments[3] : null;
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    try {
      ...
      const uniqueId = userId ? userId : uuidv4();
  ...
  useEffect(() => {
    ...
    getData();
  }, [form, id, suppliers.length > 0, offices.length > 0, editId]);
                  ...
                  <table className="min-w-full bg-white border-r border-gray1">
                    ...
                    <tbody>
                      {modulePermissions.map((module: any, index) => (
                        <tr key={module.id} className={"border-b border-gray1"}>
                        ...
                        <div className="relative">
                          <Select
                            onValueChange={(value) => {
                              ...
                              if (
                                value === "9f523136-72ca-4bde-88e5-de175bc2fc71"
                              ) {
                                form.setValue("is_boligkonfigurator", true);
                      ...
                      <FormControl>
                        <div className="relative">
                          <Select
                            ...
                            value={field.value}
                            onOpenChange={(open) => {
                              setIsOfficeDropdownOpen(open);
                              setDropdownOpen(open);
                            }}
                            open={isOfficeDropdownOpen}
                            disabled={field.value ? true : false}
                          >
                            <SelectTrigger
                              className={`bg-white rounded-[8px] border text-black ${
                            ...
                            <SelectContent className="bg-white">
                              <SelectGroup>
                                {offices?.map((off: any, index) => {
                                  return (
                                    <SelectItem value={off?.id} key={index}>
                                      {off?.data?.name}
                                    </SelectItem>
                                  );
                                })}
